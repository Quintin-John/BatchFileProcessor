...
public sealed class WatermarkTests
{
    ...
    {
        var watermark = new Watermark("g266.dat", 1200, 5, 2);
    ...
    {
        Assert.ThrowsAny<ArgumentException>(() => new Watermark(sourceKey!, 0, 0, 0));
    }
    ...
    {
        Assert.Throws<ArgumentOutOfRangeException>(() => new Watermark("src", byteOffset, lastRecordSeq, batchSeq));
    }

    [Fact]
    public void Equality_ByValue()
    {
        Assert.Equal(new Watermark("s", 1, 2, 3), new Watermark("s", 1, 2, 3));
        Assert.NotEqual(new Watermark("s", 1, 2, 3), new Watermark("s", 9, 2, 3));
    }
}
