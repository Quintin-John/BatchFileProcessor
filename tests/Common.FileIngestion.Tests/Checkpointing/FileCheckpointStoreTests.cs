...
public sealed class FileCheckpointStoreTests : IDisposable
{
    ...
    {
        var store = Store();
        var watermark = new Watermark("ABC123", 1200, 1, 0);

        await store.SaveAsync(watermark, CancellationToken.None);
        var loaded = await store.LoadAsync("ABC123", CancellationToken.None);

        Assert.Equal(watermark, loaded);
    }

    [Fact]
    public async Task MultipleFiles_HaveIndependentWatermarks()
    {
        var store = Store();

        await store.SaveAsync(new Watermark("FILE1", 100, 1, 0), CancellationToken.None);
        await store.SaveAsync(new Watermark("FILE2", 200, 2, 1), CancellationToken.None);

        Assert.Equal(100, (await store.LoadAsync("FILE1", CancellationToken.None))!.ByteOffset);
        Assert.Equal(200, (await store.LoadAsync("FILE2", CancellationToken.None))!.ByteOffset);
    }
    ...
    public async Task Save_Overwrites_PreviousWatermark()
    {
        ...
        await store.SaveAsync(new Watermark("F", 100, 1, 0), CancellationToken.None);
        await store.SaveAsync(new Watermark("F", 300, 3, 1), CancellationToken.None);

        Assert.Equal(300, (await store.LoadAsync("F", CancellationToken.None))!.ByteOffset);
    }
    ...
    public async Task Clear_RemovesWatermark()
    {
        ...
        await store.ClearAsync("F", CancellationToken.None);

        Assert.Null(await store.LoadAsync("F", CancellationToken.None));
    }
    ...
    [Fact]
    public async Task Load_BlankFileId_Throws()
    {
        await Assert.ThrowsAnyAsync<ArgumentException>(() => Store().LoadAsync("  ", CancellationToken.None));
    }

    [Fact]
    public async Task Save_FileIdWithInvalidPathChars_Throws()
    {
        var store = Store();

        await Assert.ThrowsAsync<ArgumentException>(
            () => store.SaveAsync(new Watermark("bad/id", 0, 0, 0), CancellationToken.None));
    }
}
