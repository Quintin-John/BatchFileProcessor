...
| **Q3** | **Batch size N & max message bytes — per transport.** Records/message and payload cap vs **RabbitMQ** frame limits **and Azure Service Bus max message size** (256 KB Standard / 100 MB Premium). | Throughput, memory, broker limits; ASB cap may force smaller batches than RabbitMQ. | Conservative N + payload-byte cap, **resolved per-transport from the profile**; configurable. |
...
| **Q6** | **HEAD/TRAI policy** — validate control totals (body count vs trailer) fail-closed? Publish/drop HEAD & TRAI? Reconcile the `000000043` field? | Data-integrity control; currently deferred. | Deferred — parse & ignore for now, no reconciliation. |
...
| ~~Q8~~ | **RESOLVED** — **Watermark required.** Persist last-confirmed byte offset (+ recordSeq) durably; on restart, resume from the watermark. Records in the in-flight window between watermark and crash are re-published → downstream dedupe (Q7) absorbs duplicates. New component: `ICheckpointStore` (§10). | — | — |
...
| **Q13** | **OPEN — Telemetry-backend outage policy** (§6.1) — on a *hard* Datadog/Prometheus outage, keep **blocking** ingestion (preserves "never drop lineage") or **spill** lineage to a local durable buffer and drain later? | Availability vs the full-lineage guarantee during a backend outage. | Block (consistent with "full per-record, always"); spill is the alternative if availability must win. |
